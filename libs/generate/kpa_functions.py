...
def load_kpm_data(
    gold_data_dir: str, subset: str, n_rows=None
):
    ...
    dataset_df = pl.DataFrame({'arg_id':argument_id,
                               'key_point_id':keypoint_id,
                               'argument':argument,
                               'keypoint':keypoint,
                               'topic' : topic,
                               'stance': stance})
    dataset_df=dataset_df.with_columns(pl.concat_str([pl.col("argument"),pl.col("topic")],separator=" ").alias("arg_topic"))
